angular.module('commuter.stripeservices', [])
...
.factory('StripeCharge', function($q, $http, StripeCheckout) {
   ...
   */
  self.chargeUser = function(stripeToken, selectAmmount) {
    var qCharge = $q.defer();
    ...
    var param = {
      source: stripeToken,
      amount: Math.floor(selectAmmount*100), // amount in cents
      currency: "eur",
   ...
   */
  self.getStripeToken = function(selectAmmount) {
    var qToken = $q.defer();

    var handlerOptions = {

        amount: Math.floor(selectAmmount*100),
